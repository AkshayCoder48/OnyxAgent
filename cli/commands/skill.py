...
# ------------------------------------------------------------------
@skill.command("list")
@click.option("--remote", is_flag=True, help="Browse skills on ClawHub")
@click.option("--page", default=1, type=int, help="Page number for remote listing")
def skill_list(remote, page):
    """List installed skills or browse ClawHub."""
    if remote:
...
def _list_remote(page: int = 1):
    """List skills from ClawHub v1 API (cursor-based pagination)."""
    # ClawHub v1 uses cursor-based pagination
    params = {"limit": _REMOTE_PAGE_SIZE, "sort": "recommended"}
    if page > 1 and hasattr(_list_remote, '_cursors'):
        cursor = _list_remote._cursors.get(page)
        if cursor:
    ...
    try:
        ...
        click.echo(f"Error: Failed to fetch skills from ClawHub: {e}", err=True)
        sys.exit(1)
    ...
    if next_cursor:
        if not hasattr(_list_remote, '_cursors'):
            _list_remote._cursors = {}
        _list_remote._cursors[page + 1] = next_cursor

    if not items and page == 1:
        click.echo("No skills available on ClawHub.")
        return

    installed = set(load_skills_config().keys())

    name_w = max((len(s.get("slug", "")) for s in items), default=4)
    name_w = max(name_w, 4) + 2

    click.echo(f"\n  ClawHub — page {page} (sorted by recommended)\n")
    click.echo(f"  {'Name':<{name_w}} {'Status':<12} {'Description'}")
    click.echo(f"  {'─' * (name_w + 12 + 50)}")

    for s in items:
        slug = s.get("slug", "")
        display = s.get("displayName", slug)
        desc = s.get("summary", "") or ""
        if len(desc) > 50:
            desc = desc[:47] + "..."
        status = click.style("installed", fg="green") if slug in installed else "—"
        click.echo(f"  {slug:<{name_w}} {status:<12} {desc}")

    click.echo()
    if next_cursor:
        click.echo(f"  Next page: onyx skill list --remote --page {page + 1}")
    click.echo(f"  Install:   onyx skill install <slug>")
...
def search(query):
    ...
    try:
        resp = requests.get(f"{CLAWHUB_API}/search", params={"q": query}, timeout=10)
        resp.raise_for_status()
        data = resp.json()
    except Exception as e:
        click.echo(f"Error: Failed to search skills on ClawHub: {e}", err=True)
        sys.exit(1)
    ...
    installed = set(load_skills_config().keys())
    name_w = max(len(s.get("slug", "")) for s in results)
    name_w = max(name_w, 4) + 2
    ...
    click.echo(f"\n  Install with: onyx skill install <slug>\n")
...
def install_skill(name: str) -> InstallResult:
    ...
    Accepts all formats: ClawHub name, owner/repo, GitHub/GitLab URL,
...
@click.argument("name")
def install(name):
    """Install skill(s) from ClawHub, GitHub, GitLab, git URL, or local path.
    ...
    Examples:
      ...
      onyx skill install pptx                          (from ClawHub)
...
def _install_hub(name, result: InstallResult, provider=None):
    """Install a skill from ClawHub v1 API.

    Flow:
      1. GET /api/v1/skills/{slug} → skill info + latest version
      2. GET /api/v1/download?slug={slug}&version={version} → zip
      3. Extract zip into skills_dir
    """
    skills_dir = get_skills_dir()
    os.makedirs(skills_dir, exist_ok=True)

    # Step 1: Fetch skill metadata
    result.messages.append(f"Fetching skill info for '{name}' from ClawHub...")
    try:
        info_resp = requests.get(
            f"{CLAWHUB_API}/skills/{name}",
            timeout=15,
        )
        info_resp.raise_for_status()
    except requests.HTTPError as e:
        if e.response is not None and e.response.status_code == 404:
            raise SkillInstallError(f"Skill '{name}' not found on ClawHub.")
        raise SkillInstallError(f"Failed to fetch skill info from ClawHub: {e}")
    except requests.RequestException as e:
        raise SkillInstallError(f"Failed to connect to ClawHub: {e}")

    info_data = info_resp.json()
    skill_info = info_data.get("skill", {})
    hub_display_name = skill_info.get("displayName", name)
    latest = info_data.get("latestVersion", {})
    ...
    result.messages.append(f"Found '{hub_display_name}' (v{version}) on ClawHub")
    ...
    try:
        ...
        raise SkillInstallError(f"Failed to download skill from ClawHub: {e}")

    content_type = dl_resp.headers.get("Content-Type", "")

    # Step 3: Install
    if "application/zip" in content_type or "application/octet-stream" in content_type or dl_resp.content[:2] == b'PK':
        installed_before = len(result.installed)
        _install_zip_bytes(dl_resp.content, name, skills_dir, result=result, source_label="clawhub", display_name=hub_display_name)
        if len(result.installed) == installed_before:
            _register_installed_skill(name, source="clawhub", display_name=hub_display_name)
            result.installed.append(name)
            result.messages.append(f"Installed '{name}' (v{version}) from ClawHub.")
        return

    raise SkillInstallError(f"Unexpected response from ClawHub (Content-Type: {content_type}).")
