...
class OnyxCliPlugin(Plugin):
    ...
    def _cmd_help(self, args: str, e_context, **_) -> str:
        if _t("zh", "en") == "en":
            lines = [
                ...
                "/skill list --remote: Browse ClawHub",
    ...
    def _skill_list_local(self) -> str:
        ...
        lines.append("━━━━━━━━━━━━━━━━━━━━━━━━━━")
        lines.append(_t("💡 /skill list --remote: 浏览ClawHub", "💡 /skill list --remote: Browse ClawHub"))
    ...
    def _skill_list_remote(self, page: int = 1) -> str:
        import requests
        from cli.utils import CLAWHUB_API, load_skills_config
        page_size = self._REMOTE_PAGE_SIZE
        # ClawHub v1 uses cursor-based pagination
        params = {"limit": page_size, "sort": "recommended"}
        if page > 1 and hasattr(self, '_clawhub_cursors'):
            cursor = self._clawhub_cursors.get(page)
            if cursor:
        ...
        try:
            ...
            if next_cursor:
                if not hasattr(self, '_clawhub_cursors'):
                    self._clawhub_cursors = {}
                self._clawhub_cursors[page + 1] = next_cursor
        except Exception as e:
            return _t(f"获取ClawHub失败: {e}", f"Failed to fetch ClawHub: {e}")

        if not items and page == 1:
            return _t("ClawHub暂无可用技能", "No skills available on ClawHub")

        installed = set(load_skills_config().keys())

        lines = [_t("🌐 ClawHub", "🌐 ClawHub"), ""]
        for s in items:
            slug = s.get("slug", "")
            ...
            if len(desc) > 60:
                desc = desc[:57] + "…"
            badge = _t(" [已安装]", " [installed]") if slug in installed else ""
            lines.append(f"📌 {display}{badge}")
            lines.append(_t(f"   Slug: {slug}", f"   Slug: {slug}"))
            if desc:
                lines.append(f"   {desc}")
            if downloads or stars:
                lines.append(f"   ⭐ {stars} · ⬇ {downloads}")
            lines.append("")

        lines.append("━━━━━━━━━━━━━━━━━━━━━━━━━━")
        lines.append(_t(f"📄 第 {page} 页", f"📄 Page {page}"))
        if next_cursor:
            lines.append(_t(f"💡 /skill list --remote --page {page + 1}: 下一页", f"💡 /skill list --remote --page {page + 1}: Next page"))
        lines.append(_t("💡 /skill install <slug>: 安装技能", "💡 /skill install <slug>: Install a skill"))
    ...
    def _skill_search(self, query: str) -> str:
        ...
        installed = set(load_skills_config().keys())
        lines = [_t(f"🔍 搜索「{query}」({len(results)} 个结果)", f"🔍 Search \"{query}\" ({len(results)} results)"), ""]
        for s in results:
            slug = s.get("slug", "")
            display = s.get("displayName", slug)
            desc = s.get("summary", "")
