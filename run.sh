...
# A terminal we can read from. When the script runs via a pipe, stdin is
...
# Read a line from the controlling terminal (works under piped input).
...
# When launched via process substitution or a pipe,
...
clone_project() {
    ...
    if ! command -v git &> /dev/null; then
        echo -e "${RED}❌ Git is required. Please install Git first: https://git-scm.com/downloads${NC}"
        exit 1
    fi
    ...
    if curl -sI --connect-timeout 5 --max-time 10 https://github.com > /dev/null 2>&1; then
        ...
        _timeout 60 git clone --depth 10 --progress https://github.com/AkshayCoder48/OnyxAgent.git && clone_ok=true
    fi
