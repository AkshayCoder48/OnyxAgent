...
OnyxAgent historically supported only a *single* custom provider via the flat
...
def resolve_custom_credentials():
    ...
    :return: tuple ``(api_key, api_base, model, custom_headers)``. ``api_base``,
             ``model``, and ``custom_headers`` may be ``None`` / empty when not configured.
    """
    ...
    if provider_id:
        ...
        if provider is not None:
            headers = provider.get("custom_headers") or None
