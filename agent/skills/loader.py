...
class SkillLoader:
    ...
    def _load_skill_from_file(self, file_path: str, source: str) -> LoadSkillsResult:
        ...
        if not description or not description.strip():
            # Use the skill name as a fallback description instead of skipping
            description = f"Custom skill: {name}"
            diagnostics.append(f"Skill {name} has no description in frontmatter, using default: {file_path}")
