...
class BrowserService:
    ...
    def _launch_browser(self):
        ...
        # In Docker / cloud environments (no DISPLAY), force headless + no-sandbox
        _is_docker = os.path.exists("/.dockerenv") or not os.environ.get("DISPLAY")
        if _is_docker:
            self._headless = True
        ...
        if is_cloud_deployment() or _is_docker:
            launch_args.extend([
                "--disable-gpu",
    ...
    def _do_navigate(self, url: str, timeout: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if any(indicator in page_content for indicator in [
                'cf-challenge', 'cf-browser-verification', 'cloudflare',
                'turnstile', 'challenge-platform', 'hcaptcha', 'recaptcha',
                'captcha', 'just a moment', 'checking your browser',
                'please wait', 'enable javascript'
            ]):
                cf_detected = True
        except Exception:
            pass
        ...
        if cf_detected:
            result["warning"] = (
                "Cloudflare/CAPTCHA challenge detected. The page may require human interaction. "
                "Use 'screenshot' to see the challenge, then ask the user for help. "
                "Alternative: try web_fetch tool for static content, or use web_search for a different source."
            )
        return result
