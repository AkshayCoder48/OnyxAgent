...
def _normalize_mcp_configs(raw) -> list:
    ...
    if isinstance(raw, list):
        return [cfg for cfg in raw if cfg.get("enabled", True) is not False]
...
class ToolManager:
    ...
    def _load_mcp_tools_async(self, mcp_servers_config, _is_reload=False):
        """
        ...
        If _is_reload is True, this is an incremental load from
        refresh_mcp_if_changed(); we reuse the existing registry instead
        of replacing it.
        """
        try:
            ...
            if _is_reload and self._mcp_registry is not None:
    ...
    def force_mcp_reload(self):
        """
        Force a full MCP reload regardless of file signatures.
        Used when MCP config is changed programmatically (e.g. via web UI)
        and we need to guarantee the change is picked up.
        """
        # Reset signature so refresh will detect a change
        self._mcp_signature = (None, None)

        # Now trigger the normal refresh path
