...
| `metadata.onyxagent.requires.bins` | System CLI tools that must be installed |
| `metadata.onyxagent.requires.env` | Required environment variables (all must be present) |
| `metadata.onyxagent.requires.anyEnv` | Multiple API keys — at least one must be set |
| `metadata.onyxagent.requires.anyBins` | Multiple tools — at least one must be installed |
| `metadata.onyxagent.always` | Set to `true` to always load, skipping dependency checks |
| `metadata.onyxagent.emoji` | Display emoji (optional) |
| `metadata.onyxagent.os` | OS restriction, e.g. `["darwin", "linux"]` |
