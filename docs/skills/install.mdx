...
OnyxAgent supports installing skills from [Onyx Skill Hub](https://skills.onyxagent.ai/), GitHub, ClawHub, LinkAI, and any URL via a unified `install` command. Use `/skill install` in chat or `onyx skill install` in the terminal.

## From the Skill Hub

Browse all available skills at [skills.onyxagent.ai](https://skills.onyxagent.ai/) and install by name:
...
<Tip>
  All commands above work in the terminal by replacing `/skill` with `onyx skill`. See [Skill Management Commands](/cli/skill) for full documentation.
</Tip>
