...
OnyxAgent supports installing skills from [Onyx Skill Hub](http://skills.cowagent.ai/), GitHub, ClawHub, LinkAI, and any URL via a unified `install` command. Use `/skill install` in chat or `onyx skill install` in the terminal.

## From the Skill Hub

Browse all available skills at [skills.onyxagent.ai](http://skills.cowagent.ai/) and install by name:
