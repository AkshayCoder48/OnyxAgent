...
OnyxAgent supports installing skills from [ClawHub](https://clawhub.ai/), GitHub, LinkAI, and any URL via a unified `install` command. Use `/skill install` in chat or `onyx skill install` in the terminal.

## From ClawHub

Browse all available skills at [clawhub.ai](https://clawhub.ai/) and install by name:
