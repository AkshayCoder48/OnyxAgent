...
スキル管理コマンドは OnyxAgent のスキルのインストール、検索、管理に使用します。チャットでは `/skill <サブコマンド>`、ターミナルでは `onyx skill <サブコマンド>` を使用します。
...
統一された `install` コマンドで、Onyx スキル広場、GitHub、ClawHub、任意の URL（zip アーカイブ、SKILL.md リンク）からスキルをワンクリックでインストールできます。手動ダウンロードや設定は不要です。
...
| --- | --- |
| `builtin` | プロジェクト内蔵スキル |
| `onyxhub` | OnyxAgent Skill Hub からインストール |
