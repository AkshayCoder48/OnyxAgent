...
- [ClawHub](https://clawhub.ai/)：オープンな Skill マーケットプレイス。公式推奨、コミュニティ、サードパーティの Skill を収録。ワンコマンドでインストール可能。
...
Skill のインストール：`/skill install <名前>` または `onyx skill install <名前>`。ClawHub、GitHub、URL などからインストール可能。
...
[clawhub.ai](https://clawhub.ai/) で利用可能なすべての Skill を閲覧するか、会話内でコマンドを実行できます：

```text
/skill list --remote          # ClawHub を閲覧
