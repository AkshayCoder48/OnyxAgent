---
title: 手動インストール
description: OnyxAgentの手動デプロイ（ソースコード / Docker）
---
...
<Tip>
  ネットワークに問題がある場合は、ミラーを使用してください: https://gitee.com/zhayujie/OnyxAgent
</Tip>
...
<Note>
  このステップは推奨です。インストール後、`onyx start`、`onyx stop`、`onyx update` でサービスを管理でき、`onyx skill` でスキルを管理できます。CLI をインストールしない場合は、`./run.sh` または `python3 app.py` で実行できます。
</Note>
...
| `agent_workspace` | Agent のワークスペースパス | `~/onyx` |
...
<Tip>
  すべての設定オプションはプロジェクトの [`config.py`](https://github.com/AkshayCoder48/OnyxAgent/blob/master/config.py) に記載されています。
</Tip>
