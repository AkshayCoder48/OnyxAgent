---
title: v2.1.1
description: OnyxAgent 2.1.1 - Self-Evolution, Web Console message management and parallel sessions, cross-platform MCP enhancements, new models and improvements
---

🌐 [English](https://docs.onyxagent.ai/releases/v2.1.1) | [中文](https://docs.onyxagent.ai/zh/releases/v2.1.1)

## 🧬 Self-Evolution

OnyxAgent introduces **Self-Evolution**, letting the agent go beyond completing a single task and keep improving through everyday collaboration with you:
...
<img src="https://cdn.jsdelivr.net/gh/zhayujie/onyxagent-assets@main/screenshots/en/web-console-evolution-demo.png" alt="Self-Evolution conversation example" />

Documentation: [Self-Evolution](https://docs.onyxagent.ai/memory/self-evolution)
...
Documentation: [MCP Tools](https://docs.onyxagent.ai/tools/mcp)
...
Documentation: [Models Overview](https://docs.onyxagent.ai/models)
...
- **CLI**: `onyx status` now shows the current project path
- **Hardened deployment security**: The credential-file block is narrowed to `~/.onyx/.env` so other directories are no longer affected (Thanks [@orbisai0security](https://github.com/orbisai0security) #2863); the WeChat Official Account now rejects webhook requests when `wechatmp_token` is empty
- **Group task board plugin**: Added the group task board plugin source. Thanks [@Wyh-max-star](https://github.com/Wyh-max-star) (#2853)

## 📦 Upgrade

Source-code deployments can run `onyx update` for a one-click upgrade, or pull the latest code and restart manually. See the [Upgrade Guide](https://docs.onyxagent.ai/guide/upgrade) for details.

**Release Date**: 2026.06.09 | [Full Changelog](https://github.com/zhayujie/OnyxAgent/compare/2.1.0...2.1.1)
