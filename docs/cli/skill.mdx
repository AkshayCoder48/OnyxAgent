...
Skill management commands are used to install, query, and manage OnyxAgent skills. Use `/skill <subcommand>` in chat or `onyx skill <subcommand>` in the terminal.
...
**Browse ClawHub** (view all available skills):
...
| --- | --- | --- |
| `--remote`, `-r` | Browse ClawHub remote skill list | No |
...
```bash Terminal
onyx skill search pptx
```
</CodeGroup>

## install

Install skills with a single `install` command from ClawHub, GitHub, or any URL (zip archives, SKILL.md links) — no manual download or configuration required.
...
```bash Terminal
# Install all skills in a repo (auto-discovers subdirectories with SKILL.md)
onyx skill install larksuite/cli

# Specify a subdirectory to install a single skill
onyx skill install https://github.com/larksuite/cli/tree/main/skills/lark-im

# Use # to specify a subdirectory
onyx skill install larksuite/cli#skills/lark-minutes
```
</CodeGroup>
...
```bash Terminal
onyx skill install clawhub:baidu-search
```
</CodeGroup>
...
```bash Terminal
# Install from a zip archive (single or batch)
onyx skill install https://cdn.link-ai.tech/skills/pptx.zip
...
```bash Terminal
onyx skill enable pptx
onyx skill disable pptx
```
</CodeGroup>
