...
description: OnyxAgent command system — Terminal CLI and chat commands
---

OnyxAgent provides two ways to interact via commands:

- **Terminal CLI** — Run `onyx <command>` in your system terminal for service management, skill management, and other operations
- **Chat Commands** — Type `/<command>` or `onyx <command>` in any conversation to check status, manage skills, adjust configuration, etc.

## Onyx CLI

After deploying with the one-click install script, the `onyx` command is automatically available. For manual installations, run:

```bash
pip install -e .
```

Then use the `onyx` command from anywhere:
...
Service:
  start     Start the OnyxAgent service
  stop      Stop the OnyxAgent service
  restart   Restart the OnyxAgent service
