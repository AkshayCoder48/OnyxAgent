...
The following commands can be used in chat with the `/` prefix or in the terminal with the `onyx` prefix (some are chat-only).
...
Show the current OnyxAgent version.
