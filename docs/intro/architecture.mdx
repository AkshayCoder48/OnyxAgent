---
title: Architecture
description: OnyxAgent 2.0 system architecture and core design
---

OnyxAgent 2.0 has evolved from a simple chatbot into a super intelligent assistant with Agent architecture, featuring autonomous thinking, task planning, long-term memory, and skill extensibility.

## System Architecture

OnyxAgent's architecture consists of the following core modules:

<img src="https://cdn.jsdelivr.net/gh/zhayujie/onyxagent-assets@main/architecture/en/architecture.jpg" alt="OnyxAgent Architecture" />
...
| **Skills** | Loads and manages Skills. Supports one-click installation from ClawHub, GitHub, and more, or custom skill creation through conversation |
...
| **CLI** | Command-line system providing terminal commands (`onyx`) and chat commands (`/`) for process management, skill installation, configuration, knowledge base management, and more |

## Agent Mode Workflow

When Agent mode is enabled, OnyxAgent runs as an autonomous agent with the following workflow:
...
The Agent workspace is located at `~/onyx` by default and stores system prompts, memory files, and skill files:
...
Secret keys are stored separately in `~/.onyx` directory for security:
...
| `agent_workspace` | Workspace path | `~/onyx` |
...
| `onyx_lang` | Language for the UI, command text and system prompts; `auto` to detect, or set `zh` / `en` | `auto` |
