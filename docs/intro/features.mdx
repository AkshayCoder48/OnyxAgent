---
title: Features
description: OnyxAgent long-term memory, task planning, skills system, CLI commands, and browser tool in detail
---
...
On first launch, the Agent proactively asks the user for key information and records it in the workspace (default `~/onyx`) — including agent settings, user identity, and memory files.
...
<Frame>
  <img src="https://cdn.jsdelivr.net/gh/zhayujie/onyxagent-assets@main/screenshots/en/web-console-evolution-demo.png" width="800" />
</Frame>
...
The Agent automatically organizes valuable information from conversations into knowledge pages, maintaining cross-references and indexes. The Web console provides document browsing and knowledge graph visualization. Knowledge is stored in `~/onyx/knowledge/` within the workspace.
...
- [Skill Hub](https://skills.onyxagent.ai/): An open skill marketplace featuring official, community, and third-party skills. Install with one command.
...
- **Custom skills:** Created by users through conversation, stored in the workspace (`~/onyx/skills/`), capable of implementing any complex business process or third-party integration.

Install skills: `/skill install <name>` or `onyx skill install <name>`, supporting Skill Hub, GitHub, ClawHub, URL, and more.
...
Visit [skills.onyxagent.ai](https://skills.onyxagent.ai/) to browse all available skills, or use commands in conversation:
...
OnyxAgent provides two command interaction methods, covering service management, skill installation, configuration, and more:

- **Terminal CLI:** Run `onyx <command>` in the system terminal, supporting `start`, `stop`, `restart`, `update`, `status`, `logs`, `skill`, etc.
...
onyx update             # Update and restart
onyx skill install pptx # Install a skill
onyx install-browser    # Install browser tool
```

See [Command Overview](https://docs.onyxagent.ai/en/cli) for details.
