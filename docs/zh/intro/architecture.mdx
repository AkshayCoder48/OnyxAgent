---
title: 项目架构
description: OnyxAgent 2.0 的系统架构和核心设计
---

OnyxAgent 2.0 从简单的聊天机器人全面升级为超级智能助理，采用 Agent 架构设计，具备自主思考、规划任务、长期记忆和技能扩展等能力。

## 系统架构

OnyxAgent 的整体架构由以下核心模块组成：

<img src="https://cdn.jsdelivr.net/gh/zhayujie/onyxagent-assets@main/architecture/zh/architecture.jpg" alt="OnyxAgent Architecture" />
...
| **Skills** | 加载和管理 Skills，支持从 ClawHub、GitHub 等一键安装，或通过对话创建自定义技能 |
...
| **CLI** | 命令行系统，提供终端命令（`onyx`）和对话命令（`/`），支持进程管理、技能安装、配置修改、知识库管理等操作 |

## Agent 模式

启用 Agent 模式后，OnyxAgent 会以自主智能体的方式运行，核心工作流如下：
...
Agent 的工作空间默认位于 `~/onyx` 目录，用于存储系统提示词、记忆文件、技能文件等：
...
| `onyx_lang` | 界面、命令文案、系统提示词等的语言，`auto` 自动检测，可设为 `zh` / `en` | `auto` |
