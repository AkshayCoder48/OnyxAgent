...
OnyxAgent 支持通过统一的 `install` 命令安装来自 [Onyx 技能广场](http://skills.cowagent.ai/)、GitHub、ClawHub、LinkAI 以及任意 URL 上的技能。在对话中使用 `/skill install`，在终端中使用 `onyx skill install`。

## 从Onyx技能广场安装

访问 [skills.onyxagent.ai](http://skills.cowagent.ai/) 浏览所有可用技能，找到想要的技能后直接安装，例如：
