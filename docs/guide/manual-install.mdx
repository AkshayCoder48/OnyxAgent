...
description: Deploy OnyxAgent manually (source code / Docker)
---
...
```bash
git clone https://github.com/AkshayCoder48/OnyxAgent
cd OnyxAgent/
```

<Tip>
  For network issues, use the mirror: `https://gitee.com/AkshayCoder48/OnyxAgent`
</Tip>
...
<Note>
  This step is recommended. After installation you can use `onyx start`, `onyx stop`, `onyx update` to manage the service, and `onyx skill` to manage skills. Without the CLI, you can use `./run.sh` or `python3 app.py` to run.
</Note>
...
```bash
git clone https://github.com/AkshayCoder48/OnyxAgent.git && cd OnyxAgent
```
...
| `agent_workspace` | Agent workspace path | `~/onyx` |
...
<Tip>
  Full configuration options are in the project [`config.py`](https://github.com/AkshayCoder48/OnyxAgent/blob/master/config.py).
</Tip>
