...
> 与 `channel/wechatcom/`（企微自建应用）是两个**独立的 Onyx 通道**：
>
...
回到企微后台「设置API接收」点击保存——会触发 `GET /wxkf/?...&echostr=...`，Onyx 通过 `crypto.check_signature` 校验后返回明文 `echostr`，验证成功。
